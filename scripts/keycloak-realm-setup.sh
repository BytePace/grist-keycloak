...
# Создание Keycloak realm, confidential OIDC client "grist-client" (Grist web)
# и public OIDC client для нативных приложений (PKCE).
################################################################################
...
create_or_update_oidc_mobile_client() {
    ...
    local current put_tmp put_http
